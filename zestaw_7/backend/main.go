...
func productsHandler(w http.ResponseWriter, r *http.Request) { // Funkcja obsługująca żądania skierowane pod adres /api/products
	...
	// POPRAWKA DLA LINTERA: Sprawdzamy, czy funkcja Encode nie zwróciła błędu
    err := json.NewEncoder(w).Encode(hpProducts)
    if err != nil {
        // Jeśli wystąpił błąd podczas generowania JSON, zwracamy kod 500 (Internal Server Error)
        http.Error(w, "Błąd serwera podczas generowania danych", http.StatusInternalServerError)
        return
    }
...
func main() { // Główna funkcja programu, uruchamia się automatycznie jako pierwsza
	...
	// POPRAWKA DLA LINTERA: Serwer rzuci błędem, gdy np. port 8080 będzie już zajęty
    err := http.ListenAndServe(":8080", nil)
    if err != nil {
        // Funkcja panic brutalnie, ale skutecznie zatrzymuje program i wypisuje błąd
        panic(err)
    }
